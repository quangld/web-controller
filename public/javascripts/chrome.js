...
Controller.prototype.openSettings = function () {
  ...
  this.send({
    ...
    if (data.fbManagerID) {
      $('input#facebookID').val(data.fbManagerID)
    }
    if (data.upTimeout) {
      $('input#upTimeout').val(data.upTimeout)
    }
    // facebookIDChanged
    $('input#facebookID').change(function (e) {
      _this.facebookIDChanged(e)
    })
    // autoStopChanged
    $('input#upTimeout').change(function (e) {
      _this.upTimeoutChanged(e)
    })
...
/* When Add button in Room is clicked */
...
/* When Update button in Room is clicked */
...
/* When Delete button in Room is clicked */
...
/* When Cancel button in Room is clicked */
...
/* common function to send update command to server */
Controller.prototype.updateSettings = function (info, func) {
  var _this = this
  info = $.extend(info, {
    type: 'update'
  })
  this.send(info, function (result) {
    if (result.err) {
      _this.setStatus(result.message, 'danger', 4000)
    } else {
      _this.setStatus(result.message, 'success', 4000)
...
Controller.prototype.facebookIDChanged = function (e) {
  var id = e.target.value // get value id from input
  this.updateSettings({
    facebook: id
  })
}

Controller.prototype.upTimeoutChanged = function (e) {
  console.log(e)
  var upTimeout = e.target.value // get value from input
  this.updateSettings({
    upTimeout: upTimeout * 1000
  })
}
